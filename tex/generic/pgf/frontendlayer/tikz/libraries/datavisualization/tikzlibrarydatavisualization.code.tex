...
\def\tikz@lib@dv@parse@loop{%
  \afterassignment\tikz@lib@dv@parse@@loop\let\pgf@let@token=%
}

\def\tikz@lib@dv@parse@@loop{%
  \pgfutil@switch\pgfutil@ifx\pgf@let@token{%
    {d}      {\let\pgfutil@next\tikz@lib@dv@handle@data}%
    {;}      {\let\pgfutil@next\tikz@lib@dv@parse@end}%
    {\par}   {\let\pgfutil@next\tikz@lib@dv@handle@par}%
    {s}      {\let\pgfutil@next\tikz@lib@dv@handle@beginscope}%
    {i}      {\let\pgfutil@next\tikz@lib@dv@handle@info}%
    {[}      {\let\pgfutil@next\tikz@lib@dv@handle@options}%]
    {\egroup}{\let\pgfutil@next\tikz@lib@dv@handle@endscope}%
  }{\tikz@resetexpandcount\pgfutil@next}{\tikz@lib@dv@expand}%
}

\def\tikz@lib@dv@expand{%
  % \pgferror{\meaning\pgf@let@token}%
  \advance\tikz@expandcount by -10 % go down quickly
  ...
  {%
    ...
    \expandafter\tikz@lib@dv@parse@loop\pgf@let@token
  }%
}%
...
\def\tikz@lib@dv@handle@par{\tikz@lib@dv@parse@loop}%
...
\def\tikz@lib@dv@handle@beginscope cope{%
  \begingroup%
...
\def\tikz@lib@dv@handle@data ata{%
...
\def\tikz@lib@dv@handle@info nfo{%
