...
\def\pgfmathdeclarefunction{%
  \let\pgfmath@laziness\undefined%
  \pgfmathdeclarefunction@body
}
\def\pgfmathdeclarefunction@body{%
  \pgfmath@ifnextchar*\pgfmathdeclarefunction@force\pgfmathdeclarefunction@check
}
\def\pgfmathdeclarefunction@check#1{%
    \expandafter\ifx\csname pgfmath@function@#1\endcsname\relax
        \ifcsname pgfmath#1\endcsname
            \pgfmath@error{The control sequence `\string\pgfmath#1` already exists. Choose another function name}{}%
        \fi
    \else%
...
\def\pgfmath@declarefunction#1#2{%
    ...
    % flag \pgfmath@operator@<name>@laziness is defined only if <name> is lazy
    \pgfmath@iflazy{%
      \csname pgfmath@operation@#1@laziness\endcsname
    }{%
      \expandafter\let\csname pgfmath@operation@#1@laziness\endcsname\undefined
    }%
...
% Declare a LAZY function for the parser.
%
% Works much like \pgfmathdeclarefunction except \pgfmath<name>@ will accept
% the same, uncalculated arguments as \pgfmath<name>.
\def\pgfmathdeclarelazyfunction{%
  \let\pgfmath@laziness\pgfmath@empty
  \pgfmathdeclarefunction@body
}
