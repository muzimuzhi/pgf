...
% Now eTeX \ifdefined is avialable
\ifdefined\PackageWarning
  \def\pgfkeys@warning{\PackageWarning{pgfkeys}}%
\else
  \def\pgfkeys@warning#1{\immediate\write-1{%
    Package pgfkeys Warning: #1 on input line \the\inputlineno.}}%
\fi
...
\def\pgfkeys@case@one{%
  ...
  {\pgfkeys@check@main@key{\pgfkeyscurrentkey}%
...
% #1 = full key without subkey
\def\pgfkeys@check@main@key#1{%
  \pgfkeysifdefined{#1}
    {\pgfkeys@error{Key `#1' already defined to store values}}
    {}%
}

% #1 = full key without subkey
\def\pgfkeys@check@cmd@subkey#1{%
  \pgfkeysifdefined{#1/.@cmd}
    {\pgfkeys@error{Key `#1' already defined to execute commands}}
    {}%
}
...
\pgfkeys{/handlers/.add code/.code 2 args=%
  ...
  {%
    ...
    % support for \pgfkeysdefargs:
      \pgfkeys@check@main@key{\pgfkeyscurrentpath}%
  ...
  {%
    ...
    \pgfkeys@check@main@key{\pgf@expanded@path}%
...
\pgfkeys{/handlers/.initial/.code=%
  \pgfkeys@check@cmd@subkey{\pgfkeyscurrentpath}%
